<?php
  ...
  $consulta = "SELECT CC.nome as categoria, count(V.codigo) as quantidade
  FROM Venda AS V
  INNER JOIN Cliente AS C ON (C.codigo = V.FK_Cliente_codigo)
  INNER JOIN Item_Cat_Cliente AS ICC ON (ICC.FK_Cliente_codigo = C.codigo)
  INNER JOIN Categoria_Cliente AS CC ON (CC.codigo = ICC.FK_Categoria_Cliente_codigo)
  WHERE V.empresa = '{$_SESSION["empresa"]}'
  GROUP BY CC.codigo, ICC.FK_Cliente_codigo;";
