<?php
  ...
  $consulta = "SELECT P.nome as produto, sum(IV.quantidade) as quantidade, sum(IV.valor*IV.quantidade) as total
  FROM Categoria_Produto AS CP
  INNER JOIN Item_Cat_Produto AS ICP ON (ICP.FK_Categoria_Produto_codigo = CP.codigo)
  INNER JOIN Produto AS P ON (P.codigo = ICP.FK_Produto_codigo)
  INNER JOIN Item_Venda AS IV ON (IV.FK_Produto_codigo = P.codigo)
  WHERE IV.empresa = '{$_SESSION["empresa"]}'
  GROUP BY IV.FK_Produto_codigo;";
